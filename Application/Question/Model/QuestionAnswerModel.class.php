...
class QuestionAnswerModel extends Model
{
    ...
    public function getListByMapPage($map, $page = 1, $order = 'support desc,create_time desc', $r = 10, $field = '*')
    {
        ...
        if ($totalCount) {
            ...
            foreach ($list as &$val) {
                ...
                $val['user'] = query_user(array('uid','avatar128', 'space_url', 'nickname'), $val['uid']);
    ...
    public function eachAnswer()
    {
        $rankModel = D('Question/QuestionRank');
        $uids = $this->distinct(true)->field('uid')->select();
        $uids = array_column($uids,'uid');
        $data = array();
        foreach ($uids as $v) {
            $data['uid'] = $v;
            $data['answer_count'] = $this->where(array('uid'=>$v,'status'=>1))->count();
            $data['support_count'] = $this->where(array('uid'=>$v,'status'=>1))->sum('support');
            if ($rankModel->getRank($v)) {
                D('Question/QuestionRank')->where(array('uid'=>$v))->save($data);
            } else {
                $rankModel->addData($data);
            }
        }
    }
} 