...
class QuestionController extends AdminController
{
    ...
    public function category()
    {
        ...
        $builder->title(L('_PROBLEM_CLASSIFICATION_MANAGEMENT_'))
            ->suggest(L('_DISABLE_DELETE_AND_TRANSFER_THE_PROBLEM_TO_THE_DEFAULT_CATEGORY_UNDER_CLASSIFICATION_'))
    ...
    public function add($id = 0, $pid = 0)
    {
        ...
        if (IS_POST) {
            ...
            $builder->title($title . L('_CLASSIFICATION_'))
                ...
                ->keySelect('pid', L('_FATHER_CLASSIFICATION_'), L('_SELECT_THE_PARENT_CLASS_'), array('0' => L('_TOP_CLASSIFICATION_')) + $opt)->keyDefault('pid', $pid)
    ...
    public function config()
    {
        ...
        $builder->keyRadio('QUESTION_NEED_AUDIT', L('_IS_THE_PROBLEM_NEED_TO_BE_REVIEWED_'), '', array('0' => '否', '1' => '是'))->keyDefault('QUESTION_NEED_AUDIT', 1)
            ->keyInteger('QUESTION_ANSWER_MIN_NUM',L('_QUESTION_ANSWER_AT_LEAST_NUMBER_OF_WORDS_'))->keyDefault('QUESTION_ANSWER_MIN_NUM',20)
            ->keyInteger('QUESTION_ANSWER_LIMIT_TIME','限期回答问题时间（天）')->keyDefault('QUESTION_ANSWER_LIMIT_TIME',3)

            ->keyText('QUESTION_SHOW_TITLE', '标题名称', L('_IN_THE_HOME_PAGE_SHOW_BLOCK_OF_THE_TITLE_'))->keyDefault('QUESTION_SHOW_TITLE', L('_HOT_ISSUE_'))
            ->keyText('QUESTION_SHOW_COUNT', L('_SHOW_THE_NUMBER_OF_QUESTIONS_'), L('_ONLY_AFTER_THE_SITE_HAS_BEEN_ENABLED_IN_THE_HOME_PAGE_MODULE_WILL_BE_DISPLAYED_'))->keyDefault('QUESTION_SHOW_COUNT', 4)
            ->keyRadio('QUESTION_SHOW_TYPE', L('_RANGE_OF_ISSUES_'), '', array('1' => L('_BACKGROUND_'), '0' => L('_ALL_')))->keyDefault('QUESTION_SHOW_TYPE',0)
            ->keyRadio('QUESTION_SHOW_ORDER_FIELD', L('_SORT_VALUE_'), L('_DISPLAY_MODULE_DATA_SORTING_METHOD_'), array('answer_num' => L('_ANSWER_A_FEW_'), 'create_time' => L('_PUBLICATION_TIME_'), 'update_time' => L('_UPDATE_TIME_')))->keyDefault('QUESTION_SHOW_ORDER_FIELD', 'answer_num')
            ->keyRadio('QUESTION_SHOW_ORDER_TYPE', L('_SORT_OF_'), L('_DISPLAY_MODULE_DATA_SORTING_METHOD_'), array('desc' => L('_THE_REVERSE_FROM_LARGE_TO_SMALL_'), 'asc' => L('_THE_POSITIVE_FROM_SMALL_TO_LARGE_')))->keyDefault('QUESTION_SHOW_ORDER_TYPE', 'desc')
            ->keyText('QUESTION_SHOW_CACHE_TIME', L('_CACHE_TIME_'), L('_DEFAULT_600_SECONDS_IN_SECONDS_'))->keyDefault('QUESTION_SHOW_CACHE_TIME', '600')

            ->group(L('_BASIC_CONFIGURATION_'), 'QUESTION_NEED_AUDIT,QUESTION_ANSWER_MIN_NUM,QUESTION_ANSWER_LIMIT_TIME')->group(L('_HOME_PAGE_DISPLAY_CONFIGURATION_'), 'QUESTION_SHOW_TITLE,QUESTION_SHOW_COUNT,QUESTION_SHOW_TYPE,QUESTION_SHOW_ORDER_FIELD,QUESTION_SHOW_ORDER_TYPE,QUESTION_SHOW_CACHE_TIME')
    ...
    public function index($page = 1, $r = 20)
    {
        ...
        $builder->setSelectPostUrl(U('Question/index'))
            ->select('', 'status', 'select', '', '', '', array(array('id' => 0, 'value' => L('_CURRENT_PROBLEMS_ENABLE_')), array('id' => 1, 'value' => L('_NOT_AUDITED_')), array('id' => 2, 'value' => L('_AUDIT_FAILED_OR_DISABLED_'))));
        $builder->setStatusUrl(U('Question/setQuestionStatus'));
        if ($aStatus == 1) {
            $builder->buttonEnable(null, L('_AUDIT_THROUGH_'))->buttonDisable(null, L('_AUDIT_FAILURE_'))->buttonDelete(null, L('_DELETE_DIRECTLY_'));
        } elseif ($aStatus == 2) {
            $builder->buttonEnable(null, L('_ENABLE_OR_AUDIT_THROUGH_'))->buttonDelete();
        } else {
            $builder->buttonEnable()->buttonDisable()->buttonDelete()->ajaxButton(U('Question/recommend'),array('recommend'=>1),L('_SET_UP_RECOMMEND_'))->ajaxButton(U('Question/recommend'),array('recommend'=>0),L('_CANCEL_RECOMMENDATION_'));
        }
        $builder->keyId()->keyLink('title',L('_TITLE_'),'Question/Index/detail?id=###')->keyUid()->keyText('category', L('_CLASSIFICATION_'))->keyBool('is_recommend',L('_WHETHER_IT_IS_RECOMMENDED_'))->keyCreateTime()->keyUpdateTime()->keyStatus();
        ...
        if($res){
            if($recommend==1){
                ...
                foreach($ids as $val){
                    ...
                    $messageModel->sendMessage($question['uid'], L('_PROBLEM_IS_RECOMMENDED_'), '你的问题【'.$question['title'].'】被管理员设为推荐！', 'Question/Index/detail',array('id'=>$val), is_login(), 0);
                }
            }
        ...
        if($status==1){
            foreach($ids as $val){
                ...
                if($question['status']==2){
                    $messageModel->sendMessage($question['uid'], L('_THE_PROBLEM_IS_REVIEWED_'), '你的问题【'.$question['title'].'】通过了审核！！', 'Question/Index/detail',array('id'=>$val), is_login(), 2);
                }
            }
        }else if($status==0){
            foreach($ids as $val){
                ...
                if($question['status']==2){
                    $messageModel->sendMessage($question['uid'], L('_PROBLEM_AUDIT_FAILED_'), '你的问题【'.$question['title'].'】没有通过审核！！，悬赏数额已返还！', 'Question/Index/detail',array('id'=>$val), is_login(), 2);
                }
            }
        }else{
            foreach($ids as $val){
                ...
                $rs=D('Ucenter/Score')->setUserScore($question['uid'],$question['score_num'],$question['leixing'] ,'inc');
                if($question['status']==2){
                    $messageModel->sendMessage($question['uid'], L('_PROBLEM_AUDIT_FAILED_'), '你的问题【'.$question['title'].'】被管理员直接删除！！，悬赏数额已返还！', 'Question/Index/myQuestion', is_login(), 2);
                }
            }
        }
    ...
    public function setAnswerStatus($ids,$status=1)
    {
        ...
        if($status == -1){
            foreach($ids as &$v){
                M('Question')->where(array('id' => $v))->setDec('answer_num',1);
            }
        }
    ...
    public function presetQuestion()
    {
        ...
        $admin_config->title('预设问题')
            ->data($data)
            ->keyTextArea('PRESET','预设问题','以英文;分隔')
