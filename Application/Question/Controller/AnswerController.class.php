...
class AnswerController extends BaseController
{
    ...
    private function _doEdit()
    {
        ...
        if ($res) {
            ...
            $result['info'] = $title . L('_ANSWER_A_SUCCESS_WITH_EXCLAMATION_');
            if ( D('Question/QuestionRank')->getRank($user_info['uid'])) {
                D('Question/QuestionRank')->where(array('uid'=>$user_info['uid']))->setInc('answer_count',1);
            } else {
                $temp = array();
                $temp['answer_count'] = D('Question/QuestionAnswer')->where(array('uid'=>$user_info['uid'],'status'=>1))->count();
                $temp['support_count'] = D('Question/QuestionAnswer')->where(array('uid'=>$user_info['uid'],'status'=>1))->sum('support');
                $temp['uid'] = $user_info['uid'];
                D('Question/QuestionRank')->addData($temp);
            }
        } else {
