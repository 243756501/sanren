...
class EventController extends BaseController
{
    ...
    /**
     * @author 修改 <hjy@ourstu.com>.
     */
    public function getEventMember()
    {
        $aPage = POST_I('page', 'intval',1);
        $aEventId = POST_I('id', 'intval',0);
        $aState = POST_I('state', 'intval',0);
        $map['status'] = 1;
        $map['event_id'] = $aEventId;
        if (!D('Event/Event')->where($map)->find()) {
            $this->apiError('活动不存在！');
        }
        $result = D('Event/EventAttend')->where(array('event_id'=>$aEventId,'status'=>$aState))->page($aPage,10)->select();
        if ($result) {
            foreach ($result as &$v) {
                $v['user'] = D('Api/User')->getUser($v['uid']);
            }
    ...
    {
        $mid = $this->requireIsLogin();
        $aEventId = POST_I('id', 'intval');
        $aUid = POST_I('uid', 'intval');
        $aFlag = POST_I('flag', 'intval',0);
        $this->ajaxError(array('id'=>$aEventId,'uid'=>$aUid));
        $eventInfo = M('Event')->where(array('status' => 1, 'id' => $aEventId))->find();
        if(!$eventInfo) {
            $this->apiError('活动不存在！');
        }
        if($eventInfo['uid'] != $mid){
            $this->apiError('非活动发起人无权限操作');
        }
        if($aFlag){
            $res = D('Event/EventAttend')->where(array('uid' => $aUid, 'event_id' => $aEventId))->setField('status', 1);
            if($res){
                M('Event')->where(array('id' => $aEventId))->setInc('attentionCount');
                D('Common/Message')->sendMessageWithoutCheckSelf($aUid, get_nickname( $mid) . '已经通过了您对活动【' . $eventInfo['title'] . '】的报名请求', '审核通过通知', U('Event/Index/detail', array('id' => $aEventId)),$mid);
                $this->apisuccess('操作成功');
            }
        }else{
            $res = D('Event/EventAttend')->where(array('uid' => $aUid, 'event_id' => $aEventId))->delete();
            if($res){
                M('Event')->where(array('id' => $aEventId))->setDec('attentionCount');
                D('Common/Message')->sendMessageWithoutCheckSelf($aUid, get_nickname( $mid) . '拒绝了您对活动【' . $eventInfo['title'] . '】的报名请求', '审核拒绝通知', U('Event/Index/member', array('id' => $aEventId)), $mid);
                $this->apiSuccess('操作成功');
            }
        }
        $this->apiError('操作失败！');
    ...
    public function kickOut(){
        ...
        $eventInfo = M('Event')->where(array('status' => 1, 'id' => $aEventId))->find();
        if(!$eventInfo){
            $this->apiError('活动异常或不存在');
        }
        if($eventInfo['uid'] != $mid){
            $this->apiError('非活动发起者,无权限操作');
        }
        $res = M('EventAttend')->where(array('status'=>1,'event_id' => $aEventId,'uid'=>$aUid))->delete();
        if($res){
            M('Event')->where(array('id' => $aEventId))->setDec('attentionCount');
            $this->apiSuccess('操作成功');
        }else{
