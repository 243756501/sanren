...
//        $goods = D('Shop/Shop')->getList(array('field' => 'id', 'where'=>$where, 'page' => $aPage));
    ...
    public function consigneeInfo()
    {
        ...
        $map['status'] = 1;
        $aId && $map['id'] = $aId;
        $goods = M('Shop')->where($map)->find();

        if($goods['goods_num']<$num){
            $this->apiError('商品存货不足');
        }
        ...
        $my_money = $my_money = D('Member')->where(array('uid' => $mid))->field('score' . $score_type)->find();
        if ($money_need > $my_money['score' . $score_type]) {
            $this->apiError($money_type['title'].'余额不足！');
        }
        $data['uid'] = $mid;
        $data['phone'] = $aPhone;
