...
//返回群组成员信息
    ...
    public function addGroup()
    {
        ...
        if ($isEdit) {
            ...
            $this->ApiCheckAuth('Group/Index/editGroup', get_group_admin($aId), '您无编辑群组权限');
        } else {
            $this->checkActionLimit('add_group', 'Group', 0, is_login(), true);
            $this->ApiCheckAuth('Group/Index/addGroup', -1, '您无添加群组权限');
        }
        ...
        if ($isEdit) {
            ...
            if (!$result) {
                $this->apiError('创建群组失败：' . $model->getError());
            }
            $group_id = $result;
        ...
        if ($need_verify) {
            ...
            // 发送消息
            D('Common/Message')->sendMessage(1, get_nickname($mid) . "创建了群组【{$aTitle}】，快去审核吧。", '群组创建审核', 'admin/group/unverify', array(), $mid);
    ...
    public function joinGroup()
    {
        ...
        if ($type == 1) {
            ...
            $group = D('Group/Group')->where(array('status' => 1, 'id' => $aGroupId))->find();
            // 发送消息
            D('Message')->sendMessage($group['uid'], get_nickname($mid) . "请求加入群组【{$group['title']}】", '加入群组审核', U('group/Manage/member', array('group_id' => $aGroupId, 'status' => 0)), $mid);
//            $list['title']='群组';
//            $list['content']= get_nickname($mid) . "请求加入群组【{$group['title']}】";
//            $list['message']='加入群组审核';
...
//            D('Api/Igt')->pushMessageToSingle(4,$list);
    ...
    public function GroupInvite()
    {
        ...
        if ($friend['follow_who']) {
            D('Common/Message')->sendMessage($toUid, get_nickname($mid) . "邀请您加入群组【{$group['title']}】  <a class='ajax-post' href='" . U('group/index/attend', array('group_id' => $aId)) . "'>接受邀请</a>", '邀请加入群组', U('group/index/group', array('id' => $aId)), $mid);
//            $list['title']='群组';
//            $list['content']= get_nickname($mid) . "邀请您加入群组【{$group['title']}】";
//            $list['message']='邀请加入群组';
//            $list['group_id']=$aId;
//            $list['message_type']='group';
//            $arr=array($toUid);
