...
class NewsController extends BaseController
{
    ...
    public function getCategory()
    {
        ...
        if ($aId) {
            $category = D('News/NewsCategory')->where(array('status' => 1, 'pid' => $aId))->page($aPage)->order('sort asc')->select();
        } else {
            $category = D('News/NewsCategory')->where(array('status' => 1, 'pid' => 0))->page($aPage)->order('sort asc')->select();

            foreach ($category as &$g) {
                $g['NewsSecond'] = D('News/NewsCategory')->where(array('status' => 1, 'pid' => $g['id']))->order('sort asc')->select();
    ...
    public function getMyNewsAll()
    {
        ...
        foreach ($News as &$c) {
            ...
            $c['user']= D('Api/User')->getUser($c['uid']);
        }
        unset($c);
    ...
    public function getNewsDetail()
    {
        ...
        //app点击查看资讯详情 会增加阅读数
        if ($News){
            $View=$NewsDetail['view']=$NewsDetail['view']+1;
            $Data=array();
            $Data['view']=$View;
            D('News/News')->where(array('status' => 1, 'id' => $aId))->save($Data);
        }
    ...
    public function sendNewsComment()
    {
        ...
        $reply ['user'] = D('Api/User')->getUser($reply['uid']);
