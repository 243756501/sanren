...
require_once(dirname(__FILE__) . '/' . '../IGtAPNPayload.php');
...
class IGtBaseTemplate
{
    ...
    function set_apnInfo($payload)
    {
        ...
        $len = strlen($payload);
        $apns=new \IGeTui\IGtAPNPayload();

        if ($len > $apns::$PAYLOAD_MAX_BYTES) {
            throw new Exception("APN payload length overlength (" . $len . ">" .$apns::$PAYLOAD_MAX_BYTES . ")");
        }
        $this->pushInfo = new PushInfo();
