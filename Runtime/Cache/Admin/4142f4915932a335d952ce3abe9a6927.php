...
<script>
    ...
    <div class="main-title">
        <h2><?php echo L("_NAVIGATION_MANAGEMENT_");?><span style="font-size: 14px;color: red">**任何修改后记得保存，否则修改无效</span></h2>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><li class="clearfix pLi" >
                ...
                <div class="pull-left nav_list">
                    <select name="nav[type][]" class="form-control nav-type" style="width: 110px;">
                        <option value="module" <?php if($nav['name']): ?>selected<?php endif; ?>><?php echo L("_SYSTEM_MODULE_");?></option>
                        <option value="custom" <?php if(!$nav['name']): ?>selected<?php endif; ?>><?php echo L("_CUSTOM_");?></option>
                    </select>
                    <select name="nav[modul][]" class="form-control module" style="width: 110px;<?php if(!$nav['name']){ ?>display:none<?php } ?>">
                        <?php foreach($module as $k=>$v){ ?>
                        <option value="<?php echo ($v["entry"]); ?>" data-remark="<?php echo ($v["summary"]); ?>" data-name="<?php echo ($v["name"]); ?>" data-icon="<?php echo ($v["icon"]); ?>" <?php if(strtolower($nav['url']) == strtolower($v['entry'])): ?>selected<?php endif; ?>><?php echo ($v["alias"]); ?></option>
                        <?php }unset($k,$v) ?>
                    </select>
                    <input name="nav[name][]" class="hide name" value="<?php echo ($nav["name"]); ?>">
                    <input name="nav[title][]" placeholder=<?php echo L("_PLEASE_ENTER_THE_TITLE_WITH_DOUBLE_");?> class="form-control title" style="width: 110px;" value="<?php echo ($nav["title"]); ?>">
                    <input name="nav[url][]" placeholder=<?php echo L("_PLEASE_ENTER_A_LINK_WITH_DOUBLE_");?>   class="form-control url" style="width: 200px;<?php if($nav['name']){ ?>display:none<?php } ?>" value=" <?php echo ($nav["url"]); ?>">
                </div>
                <!--<div class='pull-left' title=<?php echo L("__WITH_DOUBLE_");?>><?php $icon = $nav['icon']; if(empty($nav['icon'])){ $icon = '';} ?>
                    <select name="nav[icon][]" title=<?php echo L("__WITH_DOUBLE_");?> class="select-os-icon" data-value="<?php echo ((isset($icon) && ($icon !== ""))?($icon):'-'); ?>"></select>
                </div>-->
                ...
                <div class='pull-left color-chose hide' title='选择图标颜色' style="text-align: center">
                    <input name="nav[icon_color][]" class='simple_color_callback' value="<?php echo ((isset($nav['icon_color']) && ($nav['icon_color'] !== ""))?($nav['icon_color']):'#000000'); ?>"/>
                    <div>图标颜色</div>
                </div>
                <div class='pull-left color-chose hide' title='选择文字颜色'>
                    <input name="nav[title_color][]" class='simple_color_callback' value="<?php echo ((isset($nav['title_color']) && ($nav['title_color'] !== ""))?($nav['title_color']):'#000000'); ?>"/>
                    <div>文字颜色</div>
                </div>
                <div class='pull-left band-text'>
                    <input name="nav[remark][]"  class="form-control remark" placeholder="备注信息" value="<?php echo ($nav['remark']); ?>">
                </div>
                <div class='pull-right status-view'>
                    <input name="nav[status][]" class="status_input" value="<?php echo ($nav['status']); ?>" style="display: none;">
                    <label title='是否启用'>
                        <input class="status" <?php if($nav['status'] == 1): ?>checked<?php endif; ?>  type="checkbox" value="1">开启
                    </label>
                </div>
                <div class='pull-right new-blank' <?php if($nav['type'] == 'module'): ?>style="display:none"<?php endif; ?>>
                    <input name="nav[target][]" class="target_input" value="<?php echo ($nav['target']); ?>">
                    <label title=<?php echo L("_THE_NEW_WINDOW_OPENS_WITH_DOUBLE_");?> >
                        <input class="target" <?php if($nav['target'] == 1): ?>checked<?php endif; ?>  type="checkbox" value="1"><?php echo L("_NEW_WINDOW_OPENS_");?>
                    </label>
                </div>
        ...
        <li class="clearfix pLi" >
            ...
            <div class="pull-left nav_list">
                ...
                <select name="nav[module][]" class="form-control module" style="width: 110px;">
                    <?php foreach($module as $k=>$v){ ?>
                    <option value="<?php echo ($v["entry"]); ?>" data-remark="<?php echo ($v["summary"]); ?>" data-name="<?php echo ($v["name"]); ?>" data-icon="<?php echo ($v["icon"]); ?>" ><?php echo ($v["alias"]); ?></option>
                    <?php }unset($k,$v) ?>
                </select>
                <input name="nav[name][]" class="hide name" value="<?php echo ($module["0"]["name"]); ?>">
                <input name="nav[title][]" placeholder=<?php echo L("_PLEASE_ENTER_THE_TITLE_WITH_DOUBLE_");?> class="form-control title" style="width: 110px;" value="<?php echo ($module["0"]["alias"]); ?>">
                <input name="nav[url][]" placeholder=<?php echo L("_PLEASE_ENTER_A_LINK_WITH_DOUBLE_");?>  class="form-control url" style="width: 200px;display:none" value="<?php echo ($module["0"]["entry"]); ?>">
            </div>
            <!--<div class='pull-left' title=<?php echo L("_SELECT_ICON_WITH_DOUBLE_");?>>-->
                <!--<select name="nav[icon][]" title=<?php echo L("_SELECT_ICON_WITH_DOUBLE_");?> class="select-os-icon" data-value="<?php echo ($module["0"]["icon"]); ?>"></select>-->
            <!--</div>-->
            <div class='pull-left i-list'>
                <a href="javascript:" title=<?php echo L("_ADD_A_NAVIGATION_WITH_DOUBLE_");?>  class="add-one"><i class="icon icon-plus"></i></a>
                <a href="javascript:" title=<?php echo L("_REMOVE_THIS_NAVIGATION_WITH_DOUBLE_");?> class="remove-li"><i class="icon icon-remove"></i></a>
            ...
            <div class='pull-left band-text'>
                <input name="nav[remark][]" class="form-control remark" placeholder="备注信息"  value="<?php echo ($module[0]["summary"]); ?>">
            </div>
            <div class='pull-right status-view'>
...
<script type="text/javascript">
    ...
    <script src="/shanren/Application/Admin/Static/bootstrap/plugins/zui.js"></script>
    <script src="/shanren/Application/Api/View/default/Api/navctrl.js"></script>
    <link href="/shanren/Public/static/os-icon/select-os-icon.css" rel="stylesheet">
    <script src="/shanren/Public/static/os-icon/select-os-icon.js"></script>
    <script>
        $(function(){
