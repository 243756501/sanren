...
<script>
    ...
    <div style= "margin-bottom: 10px;<?php if(($show) == ""): ?>display: none;<?php endif; ?>" id="search_form">
    ...
    <form id="searchForm" method="get" action="<?php echo ((isset($searchPostUrl) && ($searchPostUrl !== ""))?($searchPostUrl):$default_url); ?>" class="form-dont-clear-url-param">
        <div class="search-form  cf " style="margin-bottom: 10px">
            <table class="tb_search">
                ...
                <tr><td></td>
                    <td><input type="submit" class="btn btn-default" value=<?php echo L("_DETERMINATION_WITH_DOUBLE_");?>/> <button class="btn ajax-post btn" onclick="toggle_search()"><?php echo L("_CLOSE_");?></button></td>
                    <td></td>
                </tr>
            </table>
        </div>
    ...
    <div class="with-padding adminlte-os-table">
        <table id="table-data" class="table table-bordered table-striped table-hover">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$e): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <?php if(is_array($keyList)): $i = 0; $__LIST__ = $keyList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><td style="display: none"><?php echo preg_match_all('/href/i', $e[$field['name']], $m); ;?></td>

                        <?php if(mb_strlen($e[$field['name']],'UTF8') < 30 ): ?><td  style="width:auto;max-width: <?php echo ($field['width']); ?>;" class="text-ellipsis" <?php if(($field["opt"]) == "tooltip"): ?>data-title="<?php echo ($e[$field['name']]); ?>" data-role="watch"<?php endif; ?>><?php echo ($e[$field['name']]); ?></td>
                            <?php elseif($m[0] != null): ?>
                            <td  data-role="tdclick" style="width:auto;max-width: <?php echo ($field['width']); ?>;" class="text-ellipsis" <?php if(($field["opt"]) == "tooltip"): ?>data-title="<?php echo ($e[$field['name']]); ?>" data-role="watch"<?php endif; ?>><?php echo ($e[$field['name']]); ?></td>

                            <?php else: ?>
                            <td   title="双击显示全部内容" data-role="tdclick" style="width:auto;max-width: <?php echo ($field['width']); ?>;" class="text-ellipsis" <?php if(($field["opt"]) == "tooltip"): ?>data-title="<?php echo ($e[$field['name']]); ?>" data-role="watch"<?php endif; ?><a href="<?php echo U('index/index');?>"><?php echo ($e[$field['name']]); ?></a></td><?php endif; endforeach; endif; else: echo "" ;endif; ?>
    ...
    <script type="text/javascript" src="/shanren/Application/Admin/Static/js/thinkbox/jquery.thinkbox.js"></script>
    <script type="text/javascript">
        //点击弹窗功能(解决显示不全的问题)
        $(function(){
